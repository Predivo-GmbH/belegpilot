...
This triggers a **6-step interactive workflow** producing **3 separate `.pen` files:**
- `brand-book.pen` — Layer 1: Brand identity and rules
- `design-system.pen` — Layer 2: Foundations + UI component library + composed blocks
- `mockups.pen` — Key application screens composed from Layer 2 components
...
AI generates the product logo as **standalone reusable components** in `brand-book.pen`:
...
All logo variants are stored as reusable Pencil components so they can be referenced across all `.pen` files.

---

#### Step 0.4: BRAND BOOK — Layer 1 (`brand-book.pen`)

AI builds a comprehensive brand book as frames in `brand-book.pen`. Each page is a separate top-level frame. The brand book covers the **full brand identity** — not just the UI.
...
#### Step 0.5: DESIGN SYSTEM — Layer 2 (`design-system.pen`)

AI builds the UI component library as reusable Pencil components in `design-system.pen`. This is the functional building-block layer — every component the product needs, with all variants and states, ready to be composed into screens.
...
#### Step 0.6: MOCKUP — Key Screens (`mockups.pen`)

AI generates visual mockups of the key application screens in `mockups.pen`:
- Composes screens entirely from Layer 2 components (design-system.pen)
- References logo components from Layer 1 where appropriate (sidebar, login, favicon)
- Each screen is a separate top-level frame
- You review, provide feedback, iterate, and approve before any code is written
...
**Outputs (3 files + docs):**

| Artifact | File | Purpose |
|----------|------|---------|
| Brand Book | `brand-book.pen` | Brand identity, logo rules, color/type/spacing specs, voice & tone (~18 pages) |
| Design System | `design-system.pen` | Foundations + reusable UI components + composed blocks |
| Mockups | `mockups.pen` | Key app screens composed from design system components |
...
All design artifacts are auto-loaded during Sprints 1–4. The brand-guidelines skill ensures every line of frontend code matches the approved designs.
...
- docs/DESIGN_SYSTEM.md — human-readable design spec
- Approved mockups from the design pipeline (Figma/Pencil)
  ...
  └─> PHASE 1 (Idea Validation — runs in PLAYBOOK folder)
      └─> Create new project folder? (Y/N)
          └─> PHASE 2 (Brief + Offer — runs in PROJECT folder)
              └─> PHASE 3 Step 0: /design-pipeline (brand book + design system + mockups)
